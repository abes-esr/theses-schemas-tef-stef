	 ...
	 , 1, "?"
				...
				<div class="FS1">
					...
					<div id="tef_desc_these">
						...
						<xsl:apply-templates select="key( 'dmdSec', 'tef_desc_these' )"></xsl:apply-templates>
					</div>
				...
				<div class="FS1">
					...
					<div id="tef_admin_these">
						<xsl:apply-templates select="key( 'amdSec_techMD', 'tef_admin_these' )"></xsl:apply-templates>
					</div>
					<xsl:call-template name="END">
						<xsl:with-param name="ID">tef_admin_these</xsl:with-param>
					</xsl:call-template>
					...
					<div id="MD_Rights">
						<div class="FS2">
							...
							<div id="tef_droits_etablissement_these">
								<xsl:apply-templates select="key( 'amdSec_rightsMD', 'tef_droits_etablissement_these' )/mets:xmlData/metsRights:RightsDeclarationMD"></xsl:apply-templates>
							</div>
							<xsl:call-template name="END">
								<xsl:with-param name="ID">tef_droits_etablissement_these</xsl:with-param>
							</xsl:call-template>
						...
						<div class="FS2">
							...
							<div id="tef_droits_auteur_these">
								<xsl:apply-templates select="key( 'amdSec_rightsMD', 'tef_droits_auteur_these' )/mets:xmlData/metsRights:RightsDeclarationMD"></xsl:apply-templates>
							</div>
							<xsl:call-template name="END">
								<xsl:with-param name="ID">tef_droits_auteur_these</xsl:with-param>
							</xsl:call-template>
...
-->
						...
						<xsl:for-each
							...
							<div class="FS2">
								...
								<div>
										...
										<xsl:if test="contains($admid, $mdid)">
											<div class="FS3">
												...
												<div>
													...
													<p>
														<xsl:apply-templates select="mets:mdWrap/mets:xmlData/metsRights:RightsDeclarationMD"></xsl:apply-templates>
													</p>
												</div>
												<xsl:call-template name="END">
													<xsl:with-param name="ID">MD_gestion_<xsl:value-of select="$Num_Version"/>
									...
									<xsl:if test="self::*[@TYPE = 'VERSION_INCOMPLETE'] or
										/mets:mets//mets:div[@TYPE = 'RESSOURCES_EXTERNES' or @TYPE = 'RESSOURCE_TIERS' or @TYPE='TRAVAUX']">
									<div class="FS3">
										<xsl:call-template name="BEGIN">
											<xsl:with-param name="ID">MD_descriptives_<xsl:value-of select="$Num_Version"
												/>
											</xsl:with-param>
											<xsl:with-param name="Title">Métadonnées descriptives : contenu de cette version</xsl:with-param>
											<xsl:with-param name="Level">3</xsl:with-param>
										</xsl:call-template>
										<div>
											<xsl:attribute name="id">MD_descriptives_<xsl:value-of select="$Num_Version"/>
											</xsl:attribute>
											<xsl:choose>
												<xsl:when test="self::*[@TYPE = 'VERSION_COMPLETE']">
													<xsl:call-template name="complet"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:call-template name="incomplet"/>
												</xsl:otherwise>
											</xsl:choose>
										</div>
										<xsl:call-template name="END">
									...
									<xsl:for-each select="mets:div">
										...
										<div class="FS4">
											<xsl:call-template name="BEGIN">
												...
												<xsl:with-param name="Title">Edition <xsl:choose>
													<xsl:when test="/mets:mets/mets:fileSec/mets:fileGrp[@ID=$fid]">
														(<xsl:value-of	select="/mets:mets/mets:fileSec/mets:fileGrp[@ID=$fid]/@USE"/>)
													</xsl:when>
													<xsl:otherwise>
														<xsl:text>(diffusion)</xsl:text>
													</xsl:otherwise>
												</xsl:choose>
												</xsl:with-param>
	...
	<xsl:template match="metsRights:RightsDeclarationMD">
	...
	<xsl:template match="tef:autoriteInterne" priority="0.5">
	...
	<xsl:template match="tef:MADSAuthority"/>
	<xsl:template name="complet">
		<p>Contient :</p>
		<xsl:for-each select="/mets:mets/mets:structMap//mets:div[@TYPE='RESSOURCE_TIERS' or
			@TYPE='TRAVAUX']">
			<xsl:variable name="idRextMDici">
				<xsl:value-of select="@DMDID"/>
			</xsl:variable>
			<xsl:variable name="idRextMDavant">
				<xsl:value-of select="ancestor::mets:div[@TYPE='RESSOURCES_EXTERNES']/@DMDID"/>
			</xsl:variable>
			<xsl:variable name="idRextMDDroitsici">
				<xsl:value-of select="@ADMID"/>
			</xsl:variable>
			<xsl:variable name="idRextMDDroitsavant">
				<xsl:value-of select="ancestor::mets:div[@TYPE='RESSOURCES_EXTERNES']/@ADMID"/>
			</xsl:variable>
			<ul>
				...
				<xsl:apply-templates select="/mets:mets/mets:dmdSec[@ID = $idRextMDici or @ID =
					$idRextMDavant]"/>
				<xsl:apply-templates select="/mets:mets/mets:amdSec/mets:rightsMD[@ID = $idRextMDDroitsici
					or @ID = $idRextMDDroitsavant]/mets:mdWrap/mets:xmlData/metsRights:RightsDeclarationMD"/>
			</ul>
		</xsl:for-each>
	</xsl:template>
	<xsl:template name="incomplet">
		<xsl:variable name="versionDesc">
			<xsl:value-of select="@DMDID"/>
		</xsl:variable>
		<xsl:variable name="idRext">
			<xsl:value-of select="/mets:mets/mets:dmdSec[@ID = $versionDesc]//tef:ressourceID/text()"/>
		</xsl:variable>
		<p>Manque(nt) :
			<xsl:for-each select="/mets:mets/mets:structMap//mets:div[@TYPE='RESSOURCE_TIERS' or
				@TYPE='TRAVAUX'][ancestor-or-self::mets:div/@ID = $idRext]">
				<xsl:variable name="idRextMDici">
					<xsl:value-of select="@DMDID"/>
				</xsl:variable>
				<xsl:variable name="idRextMDavant">
				...
				<ul>
					...
					<xsl:for-each select="/mets:mets/mets:dmdSec[@ID = $idRextMDici or @ID = $idRextMDavant]//tef:ressourceExterneDescription">
						<ul>
							<xsl:apply-templates/>
						</ul>
					</xsl:for-each>
					<xsl:for-each select="/mets:mets/mets:amdSec/mets:rightsMD[@ID = $idRextMDDroitsici or @ID = idRextMDDroitsavant]">
						<ul>
							<xsl:apply-templates select="mets:mdWrap/mets:xmlData/metsRights:RightsDeclarationMD"></xsl:apply-templates>
						</ul>
					</xsl:for-each>
