...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 -->
			<body background="rgb(236, 231, 246)">
				<FONT size="6">TEF</FONT>
				<h1>
					<xsl:value-of select="//tef:mainTitle"/> - <xsl:value-of select="//tef:auteur/tef:prenom"/>
									...
									<xsl:for-each select="mets:div">
										...
										<div class="FS4">
											...
											<div class="FS4">
												...
												<br/>
												<xsl:if
													test="/mets:mets/mets:fileSec/mets:fileGrp[@ID=$fid][@USE='archive']">
													<h4>
														<xsl:text>Métadonnées de fichiers :</xsl:text>
													</h4>
													<xsl:for-each
														select="/mets:mets/mets:fileSec/mets:fileGrp[@USE='archive'][@ID=$fid]/mets:file">
